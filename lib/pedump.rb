...
class PEDump
  ...
  IMAGE_FILE_HEADER = create_struct( 'v2V3v2', 2*2+4*3+2*2,
    :Machine,              # w
    :NumberOfSections,     # w
    :TimeDateStamp,        # dw
  ...
  # http://msdn.microsoft.com/en-us/library/ms809762.aspx
  IMAGE_OPTIONAL_HEADER = Struct.new(
    :Magic, # w
  ...
  class IMAGE_OPTIONAL_HEADER
    FORMAT = 'vC2V9v6V4v2V6'
    SIZE   = 2 + 2 + 4*9 + 6*2 + 4*4 + 2*2 + 6*4
    def self.read file
      new(*file.read(SIZE).unpack(FORMAT)).tap do |ioh|
        ioh.DataDirectory = []
  ...
  IMAGE_DATA_DIRECTORY = create_struct( "VV", 4*2, :va, :size, :type )
  ...
  IMAGE_SECTION_HEADER = create_struct( 'A8V6v2V', 8 + 6*4 + 2*2 + 4,
    :Name, # A8 6dw
  ...
  def dump
    File.open(@fname) do |f|
      ...
      if @mz.signature != 'MZ' && @mz.signature != 'ZM'
        log.warn "[?] no MZ header (want: 'MZ' or 'ZM', got: #{@mz.signature.inspect}"
      end
      @pe_offset = mz_hdr.unpack("@60V").first
      log.info "[.] PE offset = 0x%x" % @pe_offset

      if @pe_offset > 0x1000
        log.error "[!] PE offset is too big!"
      else
        ...
        if dos_stub_size > 0
          ...
          log.warn "[?] invalid DOS stub size = #{dos_stub_size}"
        end
      end

      f.seek @pe_offset
      @pe = PE.new(*f.read(4).unpack("a4"))
      log.warn "[?] no PE header (want: 'PE\\x00\\x00', got: #{@pe.signature.inspect})" if @pe.signature != "PE\x00\x00"
      @pe.image_file_header = IMAGE_FILE_HEADER.read(f)
      if @pe.ifh.SizeOfOptionalHeader > 0
        @pe.image_optional_header = IMAGE_OPTIONAL_HEADER.read(f)
      end
  ...
  IMAGE_RESOURCE_DIRECTORY = create_struct 'V2v4', 2*4 + 4*2,
  ...
  IMAGE_RESOURCE_DIRECTORY_ENTRY = create_struct 'V2', 2*4,
    :Name, :OffsetToData,
    :name, :data

  IMAGE_RESOURCE_DATA_ENTRY = create_struct 'V4', 4*4,
  ...
  def _read_resource_directory_tree f
    ...
    unless res_section
      log.warn "[?] can't find resource section for va=0x#{res_va.to_s(16)}"
      return []
    end
  ...
  class Resource < Struct.new(:type, :name, :file_offset, :size, :cp, :reserved, :data)
    def bitmap_hdr
    ...
    def bitmap_and_mask src_fname
      File.open(src_fname, "rb") do |f|
        ...
        "BM" + [
          ...
          0,
          BITMAPINFOHEADER::SIZE + 14 + @palette_size
        ].pack("V3") + bmp_info_hdr.to_a.pack(BITMAPINFOHEADER::FORMAT) + palette + f.read(and_mask_size)
      end
    end
  ...
  BITMAPINFOHEADER = create_struct 'V3v2V6', 3*4+2*2+6*4,
  ...
  CUR_ICO_HEADER = create_struct('v3', 3*2,
    :wReserved, # always 0
    :wResID,    # always 2
    :wNumImages # Number of cursor images/directory entries
  )

  CURDIRENTRY = create_struct 'v4Vv', 4*2+4+2,
    :wWidth,
  ...
  CURSOR_HOTSPOT = create_struct 'v2', 2*2, :x, :y

  ICODIRENTRY = create_struct 'C4v2Vv', 4+2*2+4+2,
    :bWidth,
    :bHeight,
    :bColors,
    :bReserved,
      ...
      case entry.data
        when IMAGE_RESOURCE_DIRECTORY
          if dir == @resource_directory
            ...
            resources(entry.data).each{ |res| res.name = "#{entry.name} / #{res.name}" }
          end
